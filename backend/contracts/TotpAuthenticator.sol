...
contract TotpAuthenticator is Ownable {
  ...
  function setRequest(address _target) public {
    uint256 memory _currentCount = reqestCounter;
  ...
  function setResponse(
    ...
    require(
      responses[_requestId][msg.sender] > 0,
      'Response already submitted'
    );

    AuthData _authData = AuthData(_totp5, _totp6hash, _time);
  ...
  function authenticate(
    ...
    AuthData memory _authData = responses[_responseId][_responseAddress];
    bool memory _isValid = checkHash(
      _authData.totp5,
      _lastDigit,
      _authData.totp6hash
    );
    ...
    completedAuth[_resquestId] = authentication;
  ...
  function checkHash(
    ...
    uint256 memory _totp6 = _totp5 * 10 + _lastDigit;
    // Not sure if this returns a bool?
    return sha256(_totp6) == _totp6hash;
  }
}
